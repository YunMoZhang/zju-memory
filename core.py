            ...
            def inner_wrapper(self, *arg, **kwargs):
                for i in range(times):
                    try:
                        ...
                        print("func: {:<15}\tuser: {:<10}\te: \n\n{}".format(func.__name__, self._username, "".join(traceback.format_exception(exc_type, exc_value, exc_tb))))
                        if i != times - 1:
                            continue
                        else:
...
class zju():
    ...
    def _get_grs(self, response):
        ...
        for tr in trs:
            ...
            course_semester_code = {'秋': 1, '冬': 2, '春': 3, '夏': 4}
    ...
    def _get_library(self, response):
        ...
        try:
            for tr in trs:
                tds = tr.findAll('td')
                author, book_name, year, date, return_date, place = tds[1].text.strip(',').strip('，'), tds[2].text, tds[3].text, tds[4].text, tds[6].text, tds[-1].text
    ...
    def _get_sport(self, response):
        ...
        for tr in trs:
            ...
            # 03'22 3'22
            try:
                minute, second = run.split("'")
                run = "{}'{:02d}".format(int(minute), int(second))
            except Exception:
                print(run)
                pass
        ...
        item2name = {'vital': '肺活量', 'short': '50米跑', 'jump': '立定跳远', 'stretch': '坐位体前屈', \
